...
async function resolveJidAsync(raw, sock, groupJid) {
  ...
  if (!meta) {
    try {
      ...
      if (meta?.participants) setCachedMeta(groupJid, meta);
      else meta = null;
    } catch {
      return norm;
    }
...
export async function smsg(sock, msg, store) {
  ...
  if (msg.key) {
    ...
    msg.isBot =
      ...
      /(.)\1{5,}|[^a-zA-Z0-9]|[^0-9A-F]/.test(msg.id) ||
      false;
  ...
  if (msg.message) {
    ...
    msg.msg = /viewOnceMessage|viewOnceMessageV2Extension|editedMessage|ephemeralMessage/i.test(msg.type)
    ...
    msg.args =
      msg.body
        ?.trim()
        .replace(new RegExp('^' + (msg.usedPrefix || '').replace(/[.*=+:\-?^${}()|[\]\\]|\s/g, '\\$&'), 'i'), '')
        .replace(msg.command, '')
        .split(/ +/)
        .filter((a) => a) || [];
    ...
    msg.expiration =
      msg.msg?.contextInfo?.expiration ||
      msg?.metadata?.ephemeralDuration ||
      sock?.messages?.[msg.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration ||
      0;
    ...
    if (msg.quoted) {
      ...
      msg.quoted.isBot = msg.quoted.id
        ? ['HSK', 'BAE', 'B1E', '3EB0', 'B24E', 'WA'].some((a) => msg.quoted.id.startsWith(a) && [12, 16, 20, 22, 40].includes(msg.quoted.id.length)) ||
          /(.)\1{5,}|[^a-zA-Z0-9]|[^0-9A-F]/.test(msg.quoted.id)
        : false;
      ...
      let quotedPrefix = '';
      for (const p of activePrefixes) {
        if (msg.quoted.body?.startsWith(p)) {
          quotedPrefix = p;
          break;
        }
      }
      ...
      msg.quoted.key = {
        ...
        fromMe: areJidsSameUser(sock.decodeJid(msg.msg?.contextInfo?.participant), sock.decodeJid(sock?.user?.id)),
        id: msg.msg?.contextInfo?.stanzaId
      };

      msg.quoted.fakeObj = proto.WebMessageInfo.fromObject({
        key: { remoteJid: msg.quoted.chat, fromMe: msg.quoted.fromMe, id: msg.quoted.id },
      ...
      msg.quoted.delete = () => {
        sock.sendMessage(msg.quoted.chat, {
          delete: {
            remoteJid: msg.quoted.chat,
            fromMe: msg.isBotAdmin ? false : true,
            id: msg.quoted.id,
            participant: msg.quoted.sender
          }
        });
  ...
  if (!sock.sendImageAsSticker) {
    sock.sendImageAsSticker = async (jid, p, quoted, options = {}) => {
      const buff = Buffer.isBuffer(p)
        ? p
        : /^data:.*?\/.*?;base64,/i.test(p)
        ? Buffer.from(p.split`,`[1], 'base64')
        : /^https?:\/\//.test(p)
        ? await getBuffer(p)
        : fs.existsSync(p)
        ? fs.readFileSync(p)
  ...
  if (!sock.sendCodeMessage) {
    sock.sendCodeMessage = async (jid, filename, code, quoted, tableData) => {
      ...
      function tokenize(src) {
        const tokens = [];
        let i = 0;
        const push = (content, type = 'DEFAULT') => {
          if (content) tokens.push({ content, type });
        };
        ...
        for (const token of tokens) {
          ...
          if (last?.type === 'DEFAULT' && token.type === 'DEFAULT') last.content += token.content;
      ...
      if (tableData) {
        const tableRows = [
          { items: tableData.headers, isHeading: true },
          ...tableData.rows.map((r) => ({ items: r.map(String) }))
        ];
        ...
        sections.push({
          view_model: {
            primitive: {
              ...
              rows: tableRows.map((row) => ({ is_header: row.isHeading ?? false, cells: row.items, markdown_cells: [] })),
